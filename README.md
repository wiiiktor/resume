...
2. Ukraine tweets, finding "expert tweets"
3. Names generator, Generative AI within NLP
4. Source code language recognition, based on a very short code sample
5. RNN 2-layer, implemented with raw Python & NumPy, with derivatives calculation
6. LSTM 2-layer, with no use of nn.LSTM PyTorch object
7. CNN working with batch_size=1
...
I loaded a manual with a lot of graphics placed between or next to text paragraphs. LLM's job is to create descriptions to these graphics, although obviously it does not "look" at them, but only text around. I wrote a prompt that gave 18 good results out of 19 in total (only the first photo was described incorrectly). This is a very unique prompt, as it gives captions of object that LLM has never seen, in contrary to an Eiffel Tower or Statue of Liberty :-)
...
## Ad 6. LSTM 2-layer with no use of nn.LSTM PyTorch object
...
## Ad 7. CNN working with batch_size=1
